...
export interface VisitorItemCardProps {
  visitor: VisitorItem;
  index: number;
...
export const VisitorItemCard: React.FC<VisitorItemCardProps> = ({
  visitor,
  index,
  onSaveEdit,
  onRemove,
  ...
  const handleStartEdit = () => {
    setIsEditing(true);
    const text = visitor.church ? `${visitor.name} (${visitor.church})` : visitor.name;
    setEditingText(text);
  };

  const handleSave = () => {
    if (!editingText.trim()) return;
    onSaveEdit(visitor.id, editingText);
    setIsEditing(false);
  };
  ...
  return (
    <div className="flex items-center justify-between p-3 rounded-xl bg-church-parchment/60 border border-church-sand hover:bg-church-parchment transition-colors">
      <div className="text-sm font-sans">
        <span className="font-mono text-xs font-bold text-church-gold-dark mr-1.5">{index + 1}.</span>
        <span className="font-semibold text-church-charcoal">{visitor.name}</span>
        {visitor.church && <span className="text-church-muted text-xs"> ({visitor.church})</span>}
        {visitor.invited_by && (
          <span className="text-church-muted text-xs block sm:inline sm:ml-2">
            Convidado por: {visitor.invited_by}
          </span>
        )}
      </div>
