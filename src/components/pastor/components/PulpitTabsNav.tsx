...
export const PulpitTabsNav: React.FC<PulpitTabsNavProps> = ({
  ...
  return (
    <header className="bg-white border-b-2 border-church-sand px-1 sm:px-4 py-1 sm:py-2.5 shrink-0 shadow-xs select-none">
      <div className="max-w-6xl 2xl:max-w-7xl mx-auto flex items-center justify-between gap-1 sm:gap-3">
        {/* Título da Visão no Canto Superior Esquerdo (Apenas em telas grandes) */}
        ...
        {/* 4 Botões da Faixa Superior — Calibrados para 100% de visibilidade no mobile */}
        <nav
        ...
        >
          ...
          <button
            ...
            className={`flex-1 sm:flex-initial inline-flex items-center justify-center gap-0.5 xs:gap-1 sm:gap-2 px-0.5 xs:px-1.5 sm:px-5 py-1.5 sm:py-2.5 rounded-lg sm:rounded-2xl text-[9px] xs:text-[10px] sm:text-sm font-title font-black uppercase tracking-tight sm:tracking-wider transition-all cursor-pointer min-h-[36px] sm:min-h-[50px] select-none ${
              activeTab === 'prayers'
          ...
          >
            <Heart
              className={`w-2.5 h-2.5 xs:w-3 xs:h-3 sm:w-5 sm:h-5 shrink-0 ${
                activeTab === 'prayers' ? 'text-white' : 'text-church-gold-dark'
              }`}
            />
            <span className="truncate">Orações</span>
            <span
              className={`px-1 py-0.2 rounded-full text-[8px] xs:text-[9px] sm:text-xs font-mono font-bold leading-none ${
          ...
          >
            ...
            />
            <span className="hidden sm:inline truncate">Oportunidades</span>
            <span className="sm:hidden truncate">Oport.</span>
            <span
              className={`px-1 py-0.2 rounded-full text-[8px] xs:text-[9px] sm:text-xs font-mono font-bold leading-none ${
          ...
          <button
            ...
            className={`flex-1 sm:flex-initial inline-flex items-center justify-center gap-0.5 xs:gap-1 sm:gap-2 px-0.5 xs:px-1.5 sm:px-5 py-1.5 sm:py-2.5 rounded-lg sm:rounded-2xl text-[9px] xs:text-[10px] sm:text-sm font-title font-black uppercase tracking-tight sm:tracking-wider transition-all cursor-pointer min-h-[36px] sm:min-h-[50px] relative select-none ${
              activeTab === 'alerts'
                ? 'bg-church-gold text-white shadow-md border-2 border-church-gold-dark ring-2 ring-church-gold/40 scale-[1.02]'
                : activeAlert
