...
| **1.2** | `src/context/RoomContext.tsx` (Fatia Polling) | ~300 (de 1.031) | Polling adaptativo, timers de cold start e sincronização inter-abas | ✅ **Concluído** (Motor de polling e sync isolados em `src/context/sync/` com 4 arquivos ≤ 57L) | Observer + Custom Hook | ~80 |
| **1.3** | `src/services/neon.ts` | 319 | Chamadas HTTP acopladas, helpers de retry e tokens soltos | ✅ **Concluído** (Módulos isolados em `src/services/api/` e `neon.ts` reduzido de 319L para 5L) | Repository Pattern | ~100 |
| **2.1** | `src/components/room/JoinRoomModal.tsx` | 401 | Modal duplo com lógica de entrada, criação de sala e diálogo de conflito | ✅ **Concluído** (Subcomponentes em `src/components/room/`: `RoleSelectorTabs`, `JoinRoomForm`, `CreateServiceForm`, `RoomConflictModal`, `useRoomEntryFlow` e `JoinRoomModal` reduzido para 89L) | Composition Pattern + Custom Hook | ~89 |
| **2.2** | `src/context/RoomContext.tsx` (Orquestrador) | 1.031 | Contexto inchado com 18 responsabilidades simultâneas | ✅ **Concluído** (Camadas especializadas em `mutations/`, `control/`, `session/`, `sync/`, `storage/` e `RoomContext.tsx` reduzido para 106L) | Facade + Provider Pattern | ~106 |
| **3.1** | `src/components/pastor/PulpitView.tsx` (Fatia Lógica) | ~400 (de 1.535) | Cálculos inline de particionamento, detecção KitKat e dimensões | ✅ **Concluído** (Módulos isolados em `utils/pulpitPartitioning.ts`, `hooks/usePulpitLayout.ts` e `hooks/usePulpitScroll.ts` ≤ 81L) | Pure Calculations + Custom Hook | ~80 |
| **3.2** | `src/components/pastor/PulpitView.tsx` (Fatia UI) | ~700 (de 1.535) | Folhas de visualização, banner de alerta flutuante e botões de zoom | ✅ **Concluído** (Subcomponentes modulares em `src/components/pastor/components/` com 13 arquivos ≤ 168L) | Composition + Atoms | ~110 |
| **3.3** | `src/components/pastor/PulpitView.tsx` (Orquestrador) | 1.535 | Tela master com JSX quilométrico alternando modo mobile e paisagem | ✅ **Concluído** (`PulpitView.tsx` reduzido de 1.535L para 160L) | Composition Pattern | ~130 |
| **4.1** | `src/components/obreiro/ObreiroEditor.tsx` (Fatia Estado) | ~450 (de 1.682) | 12 useStates repetitivos para rascunhos de visitantes, oração e oportunidades | ✅ **Concluído** (`useDraftBatch.ts` isolado em `src/components/obreiro/hooks/` com 123L) | Reducer Pattern + Custom Hook | ~110 |
| **4.2** | `src/components/obreiro/ObreiroEditor.tsx` (Fatia Cards) | ~800 (de 1.682) | Blocos de formulários repetitivos para 4 domínios litúrgicos | ✅ **Concluído** (Subcomponentes modulares em `src/components/obreiro/sections/` com 8 arquivos ≤ 177L) | Composition Pattern | ~120/card |
| **4.3** | `src/components/obreiro/ObreiroEditor.tsx` (Orquestrador) | 1.682 | Componente com mais de 1.600 linhas com scrollspy e cabeçalho inline | ✅ **Concluído** (`ObreiroEditor.tsx` reduzido para 160L, `useObreiroScrollSpy`, `ObreiroSectionNav` e modais modulares) | Composition Pattern | ~150 |
